...
const int MAX_EDGE_NUM = 400010;
#define MAX_LINE_LEN 300010
#define PRINT printf
const int INNER_MAX = 10010;
const int CONSUME_MAX = 10010;
const int _INT_MAX = 0x7fffff;
const int _INT_MIN = -INT_MAX;

const int SERVER_MAX = 20;
...
struct ConsumeNode {
    ...
    ConsumeNode(int consume = -1, int inner = -1, int req = -1, bool s = false) : consume_node(consume),
                                                                                  inner_node(inner), requirement(req),
                                                                                  seen(s) {}
...
struct ServerLevel {
    ...
    ServerLevel(int cp = 0, int ct = 0) : cap(cp), cost(ct) {}
};

ServerLevel serverlvl[SERVER_MAX];

int cost_table[INNER_MAX][INNER_MAX];
int capacity_table[INNER_MAX][INNER_MAX];
int inn, cnn, server_cost;
int cap_table[INNER_MAX];
int nodemap[INNER_MAX];

void init(char *topo[MAX_EDGE_NUM], int line_num) {
    // init cost_table.
    for (int i = 0; i < INNER_MAX; ++i) {
        fill(cost_table[i], cost_table[i] + INNER_MAX, _INT_MAX);
        cost_table[i][i] = 0;
    ...
    i += 2;
    int serverid, servercapicity, servercost;
    for (;; ++i) {
        stream << topo[i];
        if (strlen(topo[i]) == 2) break;
        stream >> serverid >> servercapicity >> servercost;
        stream.clear();
        serverlvl[serverid] = ServerLevel(servercapicity, servercost);
    }
    stream.clear();
    ...
    for (; i < nodenum; ++i) {
        stream << topo[i];
        stream >> nodeid >> nodecost;
        stream.clear();
        nodemap[nodeid] = nodecost;
    }
...
int check(char *res[MAX_EDGE_NUM]) {
    bool usedserver[INNER_MAX];
    fill(usedserver, usedserver + INNER_MAX, false);
    stringstream stream;
    int i = 0;
    int line = 0;
    int cost = 0;
    ...
    for (; i < 2 + line; ++i) {
        ...
        if (!usedserver[path[0]]) {
            cap_table[path[0]] = serverlvl[path[path.size() - 1]].cap;
            usedserver[path[0]] = true;
            cost += nodemap[path[0]] + serverlvl[path[path.size() - 1]].cost;
        }
        cap_table[path[0]] -= flow;
        if (cap_table[path[0]] < 0) {
            cout << "capacity oversubscribe" << endl;
            return -1;
        }
...
int main(int argc, char *argv[]) {
    ...
    int line_num;
    const char *topo_file = "../0/case0.txt";

    line_num = read_file(topo, MAX_EDGE_NUM, topo_file);
    const char *result_file = "../out0-0.txt";
