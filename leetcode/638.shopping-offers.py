...
class Solution:
    def shoppingOffers(self, price: List[int], special: List[List[int]], needs: List[int]) -> int:
        ...
        def directBuy(goods):
            return sum([n*price[i] for i,n in enumerate(goods)])

        def dfs(remains, pos):
            direct = directBuy(remains)

            for i in range(pos, len(special)):
                use_special = True
                next_remain = []
                for j in range(len(remains)):
                    if remains[j] < special[i][j]:
                        use_special = False
                        break
                    
                    next_remain.append(remains[j] - special[i][j])

                if use_special:
                    direct = min(direct, dfs(next_remain, i) + special[i][-1])
